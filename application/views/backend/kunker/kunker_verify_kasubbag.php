...
<div class="invoice">
	...
	<div class="invoice-footer">
		...
		<p class="text-center fw-bold">
			...
			<?php if (!in_array($dispo_kasubag_stat, [1, 2])) : ?>
			...
			<?php elseif (in_array($dispo_kasubag_stat, [1, 2])) : ?>
...
<script>
	...
	function confirm(status) {
		...
		swal({
			title: 'Yakin melanjutkan disposisi Permohonan SPPD?',
			...
			if (isConfirmed) {
				$.ajax({
					url: '<?= base_url() ?>kunker/verify_action_kasubbag',
					type: 'POST',
					data: {
						status: status,
						id_kunker: <?= @$id_kunker ?>,
